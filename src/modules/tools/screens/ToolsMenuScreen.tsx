// src/modules/tools/screens/ToolsMenuScreen.tsx
...
import { CruciBot } from '../../../ui/components/CruciBot';

const ToolCard = ({ title, description, iconName, onPress, isLocked }: any) => (
  <TouchableOpacity style={styles.cardContainer} onPress={onPress} activeOpacity={0.8}>
    <View style={[styles.iconContainer, { backgroundColor: isLocked ? '#f5f5f5' : THEME_COLORS.primary + '15' }]}>
      <MaterialCommunityIcons 
        name={isLocked ? "lock" : iconName} 
        size={28} 
        color={isLocked ? "#999" : THEME_COLORS.primary} 
      />
    </View>
...
export const ToolsMenuScreen = () => {
  ...
  useEffect(() => {
    const initializeMenu = async () => {
      ...
      if (currentSession) {
        ...
        // 3. Intentar actualizar el rol desde la nube
  ...
  return (
    <View style={styles.mainWrapper}>
      ...
      {/* El Bot solo aparece en esta pantalla */}
      <CruciBot />
    </View>
  );
};
...
const styles = StyleSheet.create({
  mainWrapper: { flex: 1 },
  mainContainer: { flex: 1, backgroundColor: THEME_COLORS.primary },
  ...
  scroll: { paddingBottom: 100 }, // Espacio para que el Bot no tape el último item
